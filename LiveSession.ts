...
export class LiveSession {
  ...
  async connect(): Promise<void> {
    ...
    const configuredServer = (import.meta.env.VITE_TUNE_SERVER_URL || "").trim().replace(/\/$/, "");
    const serverOrigin = configuredServer || window.location.origin;
    const serverUrl = new URL(serverOrigin);
    serverUrl.protocol = serverUrl.protocol === "https:" ? "wss:" : "ws:";
    const wsUrl = `${serverUrl.toString().replace(/\/$/, "")}/ws`;
